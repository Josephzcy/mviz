...
echo -e "${BLUE_COLOR}please run this shell under mviz2_ros like this: bash ./script/docker_env_release.sh${RES}"
cd `dirname $(dirname $0)`    # in mviz2_ros dir now
docker_image_dir=../mviz_melodic_1211.tar     #此处建议用绝对路径
volume_dir_path=$(pwd)

container_names="mviz_havp_vm_v2.0.10"
...
docker run -v /tmp/.X11-unix:/tmp/.X11-unix -v /usr/share/zoneinfo:/usr/share/zoneinfo -e DISPLAY=unix$DISPLAY \
          ...
          -v ${volume_dir_path}:/root/mviz2_ros  --privileged=true -it \
